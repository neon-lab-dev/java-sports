...
const AccountPageWrapper = ({ children }) => {
  ...
  return (
    <div className="bg-white w-full">
      <section className="wrapper flex items-center justify-center flex-col pt-4 gap-6 ">
        <h1 className="font-600 text-4xl">Welcome, {USER.name}</h1>
        <div className="flex justify-between w-full gap-8 lg:border-t border-grey-light py-12">
          <div className="flex-col gap-4 hidden lg:flex border-grey-light border-x-2 w-64">
            {TABS.map((tab) => (
              <button
                key={tab}
              ...
              >
                {paramToWord(tab)}{" "}
                {tab === "recent-orders" && <>({USER.recentOrders})</>}
              </button>
            ))}
          </div>
